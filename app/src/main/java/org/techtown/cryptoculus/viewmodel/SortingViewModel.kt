    ...
    fun sortCoinInfos(coinInfos: ArrayList<CoinInfo>): ArrayList<CoinInfo> {
        coinInfos.sortWith { o1, o2 ->
            when (repository.getSortMode()) {
                ...
                2 -> o1.ticker.lastInTicker.replace(",", "").toDouble().compareTo(o2.ticker.lastInTicker.replace(",", "").toDouble()) // 현재가 오름차순
                3 -> o2.ticker.lastInTicker.replace(",", "").toDouble().compareTo(o1.ticker.lastInTicker.replace(",", "").toDouble()) // 현재가 내림차순
